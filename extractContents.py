...
news = ''
#     segeu_ilbo = lambda x: x.split(re.findall('[.][가-힣]{0,2}[.][가-힣]{2,} [가-힣]*기자', x)[0]) if len(re.findall('[.][가-힣]{0,2}[.][가-힣]{2,} [가-힣]*기자', x)) != 0 else x.split('ⓒ 세상을 보는 눈, 글로벌 미디어') + str('.')
...
with open('./news_20171231.json', 'r', encoding='utf-8') as f:
    ...
    for i in range(1, cnt + 1):
        ...
        jdata['news' + str(i)]['contents'] = extract_contesnt[press](contents)
        print(contents)
